...
from database import count, DB, get_match_char_page
...
@toolz.curry
async def match_of_page(db: DB, char: MatchCharDict, offset: int) -> Iterable[Match]:
    return search(char, await get_match_char_page(db, PAGE_SIZE, offset))
...
async def find_matches(db: DB, char: Character) -> Iterable[Match]:
    ...
    return await gathercat(match_of_page(db, as_mc), offsets)
